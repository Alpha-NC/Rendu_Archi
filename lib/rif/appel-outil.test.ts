...
describe('analyserReponseModele — détection des appels fantômes (prémortem #1)', () => {
  it('reconnaît un vrai tool_use', () => {
    const resultat = analyserReponseModele([
      { type: 'tool_use', name: 'genererRenduFlux', input: { dossierId: 'd-1' } },
    ])
    expect(resultat).toEqual({
      genre: 'appel_reel',
      operation: 'genererRenduFlux',
  ...
  it("détecte un appel simulé en texte (bloc de code JSON) et ne l'exécute pas", () => {
    const resultat = analyserReponseModele([
      {
        type: 'text',
        text: 'Je lance la génération :\n```json\ngenererRenduFlux({"dossierId": "d-1"})\n```',
      },
    ])
    expect(resultat.genre).toBe('appel_simule_detecte')
    if (resultat.genre === 'appel_simule_detecte') {
      expect(resultat.operationEvoquee).toBe('genererRenduFlux')
    }
  })

  it('détecte la forme "name": "corrigerRenduFlux" imitant un schéma d\'outil', () => {
    const resultat = analyserReponseModele([
      { type: 'text', text: '{"type": "tool_use", "name": "corrigerRenduFlux"}' },
    ])
    expect(resultat.genre).toBe('appel_simule_detecte')
  })
  ...
  it('donne la priorité au tool_use réel même si du texte bavard le précède', () => {
    const resultat = analyserReponseModele([
      { type: 'text', text: 'Je vais appeler genererRenduFlux maintenant.' },
      { type: 'tool_use', name: 'genererRenduFlux', input: { dossierId: 'd-2' } },
    ])
...
describe('autoriserOperation — revérification backend (PRD §11, §14)', () => {
  it('refuse une génération hors PRÊT_À_GÉNÉRER', () => {
    const decision = autoriserOperation('genererRenduFlux', 'COLLECTE_EN_COURS', etatConfirme)
  ...
  it('autorise une génération depuis PRÊT_À_GÉNÉRER avec révision confirmée', () => {
    const decision = autoriserOperation('genererRenduFlux', 'PRET_A_GENERER', etatConfirme)
    expect(decision.autorisee).toBe(true)
  })

  it('refuse une correction locale depuis un état qui ne le permet pas', () => {
    const decision = autoriserOperation('corrigerRenduFlux', 'PRET_A_GENERER', etatConfirme)
    expect(decision.autorisee).toBe(false)
  })
  ...
  it('ne laisse pas une reprise passer pour une correction locale (D-09)', () => {
    ...
    expect(autoriserOperation('corrigerRenduFlux', 'A_REPRENDRE', etatConfirme).autorisee).toBe(
      false,
    )
    expect(
    ...
    expect(autoriserOperation('corrigerRenduFlux', 'A_CORRIGER', etatConfirme).autorisee).toBe(
      true,
    )
  })
})
