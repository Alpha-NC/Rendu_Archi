...
**Note — type de sortie, un troisième axe indépendant (ADR-022, RDV Évariste 18.09.2026) :** le mode de production (cet axe) et le style (LIB-005) sont désormais rejoints par un troisième axe, le **type de sortie** (`OutputType` : rendu perspective photoréaliste ou rendu axonométrique photoréaliste) — voir `docs/PRD_RIF_V2_GEOMETRY_FIRST.md` §14. Ces trois axes restent indépendants et se combinent librement : un type de sortie n'est ni un mode, ni un style. Le type de sortie ne change ni le mode retenu ni le style choisi ; il ne fait que réorienter les critères de contrôle qualité prioritaires (05_CHECKLIST_CONTROLE.md, note geometry-first) vers ceux pertinents pour une perspective ou pour une axonométrie.
...
V1.8 — Ajout d'une note (§1, ADR-022) : le type de sortie (`OutputType`, perspective/axonométrie) devient un troisième axe indépendant du mode et du style, sans modifier les modes existants.
