...
- 17.09.2026 — Bascule réelle sur Neon (D-19) : projet Neon provisionné et lié, `neon/schema.sql` corrigé (référence `neon_auth."user"`, plus de table `users` maison) et exécuté sur la branche `production` réelle, `obtenirDepot()` branché sur `creerDepotNeon`, authentification individuelle livrée via Neon Auth (`lib/auth/{server,client}.ts`, `proxy.ts`, `app/connexion/page.tsx`) et vérifiée contre le service hébergé réel. Code Supabase (`lib/supabase/`, `depot-supabase.ts`, schéma) laissé en place mais mort, plus aucun appelant. Reste ouvert : aucun compte utilisateur créé, `BLOB_READ_WRITE_TOKEN` manquant. 223 tests passent, tsc/build/lint propres, 0 vulnérabilité.
