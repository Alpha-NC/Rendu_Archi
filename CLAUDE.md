...
- `lib/rif/prompt-conversationnel.ts` — prompt système de l'assistant conversationnel : les 11 principes non négociables (PRD §4) + le plan de collecte déjà calculé par `collecte-conditionnelle.ts` (le modèle exécute ce plan, il ne déduit jamais lui-même les règles ENG-004) + les schémas d'outils, délibérément minimaux (`genererRenduFlux` n'accepte aucun prompt du modèle).
