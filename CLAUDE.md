...
- `app/api/dossiers/route.ts` (GET liste, POST création), `app/api/dossiers/[dossierId]/sources/route.ts` (dépôt d'une source, appelle la détection automatique) et `app/api/dossiers/[dossierId]/sources/[fileId]/route.ts` (PATCH, confirmation humaine du rôle quand la détection est ambiguë — seul endroit qui écrit `role_confirmed` à la main).
