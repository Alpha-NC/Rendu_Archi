...
- `lib/storage/vercel-blob.ts` (+ `.test.ts`) — adaptateur de stockage Vercel Blob (PRD V2.1 §16, D-19) : `televerserFichier`/`lireFichier`/`supprimerFichier`, accès `private` (vérifié dans les types du SDK avant d'écrire le module). Additif, **non encore branché** sur `DepotDossiers` ni aucune Route Handler — Supabase Storage reste le stockage réellement utilisé. Décision d'architecture non prise ici, à trancher avec `depot-neon.ts` : `@vercel/blob` n'a pas d'équivalent direct à une URL signée Supabase utilisable telle quelle par le navigateur — servir un fichier passera par une Route Handler qui lit le blob côté serveur et le retransmet.
...
- `supabase/schema-rif-app.sql` — schéma complet (`dossiers`, `files`, `generations`, `quality_audits`, `events`, `profiles`) avec policies RLS, plus le bucket de stockage privé et ses policies `storage.objects` (section 7) ; non encore exécuté sur un vrai projet Supabase — runbook : `docs/provisioning-supabase.md`.
...
- `depot-neon.ts` (Neon PostgreSQL) — implémentation de `DepotDossiers` ciblant Neon, consommant `lib/storage/vercel-blob.ts` pour les fichiers. C'est à cette étape que la question laissée ouverte ci-dessus (comment servir un fichier privé au navigateur) doit être tranchée.
...
- Créer le projet Supabase réel (action utilisateur — jamais faite depuis cet environnement, identifiants du client) : runbook complet dans `docs/provisioning-supabase.md`. `supabase/schema-rif-app.sql` crée désormais aussi le bucket de stockage privé et ses policies RLS (section 7 du script, exécutable telle quelle — nécessaire au fonctionnement, pas une simple convenance : les Route Handlers utilisent le client RLS, jamais `service_role`).
