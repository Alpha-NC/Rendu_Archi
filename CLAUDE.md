...
- `lib/rif/contraintes-libertes.ts` (+ `.test.ts`) — D-17, matrice Contraintes & Libertés (PRD §9.4A, §24A) : une propriété absente de la matrice vaut `locked` (la liberté créative n'est jamais implicite), la plus restrictive prévaut, et la géométrie d'un élément structurel ne descend jamais sous `strict`.
